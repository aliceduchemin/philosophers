...
/*   Updated: 2021/02/09 23:22:02 by aduchemi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_create_philos(int ac, char **av)
{
	...
	while (i < g_global.number_of_philosophers)
	{
		...
		g_global.philos[i].p_to_global_mutex = &(g_global.mutex);
		i++;
	}
}
...
void	ft_create_right_fork(void)
{
	...
	while (i < g_global.number_of_philosophers)
	{
		...
		previous = (i + g_global.number_of_philosophers - 1) % g_global.number_of_philosophers;
